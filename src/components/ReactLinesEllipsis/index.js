...
class LinesEllipsis extends React.Component {
  ...
  initCanvas() {
    ...
    this.copyStyleToCanvas();
    forEach(Object.keys(canvasStyle), (key) => {
  ...
  reflow(props) {
    ...
    const basedOn =
      props.basedOn || (/^[\x00-\x7F]+$/.test(props.text) ? 'words' : 'letters');
    switch (basedOn) {
      case 'words':
    ...
    this.canvas.innerHTML = map(this.units, (c) => {
    ...
    this.setState(newState, () => {
      const targetHeight = parseFloat(getStyle(this.target, 'height'));
      props.onReflow({ ...newState, targetHeight });
    });
  }
